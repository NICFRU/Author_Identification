...
# %%
df1 = df1.drop(columns=['Unnamed: 0', 'id', 'gender', 'age', 'sign', 'date', 'text',
...
# %%
df1.rename(columns = {'topic':'labels','sent_tokenize':'text'}, inplace = True)
...
#%%
df = df1.groupby('labels').head(160)
indexlist = df.index
df = df.reset_index(drop=True)
# %%
dfvali = df.groupby('labels').head(160*0.2)
# %%
...
# %%
dftrain.to_csv("train_topic.csv",index=False)
dfvali.to_csv("vali_topic.csv",index=False)
dftest.to_csv("test_topic.csv",index=False)
#%%
len(dftrain.labels.unique())
# %%
